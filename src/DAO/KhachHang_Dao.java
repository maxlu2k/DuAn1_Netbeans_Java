...
public class KhachHang_Dao implements IKhachHang<KhachHang, Object> {
    ...
    public void insert(KhachHang entity) {
        ...
        jdbcHelper.update("insert into KHACHHANG (MAKHACHHANG,HOTEN,SODT,DIACHI,EMAIL) values (?,?,?,?,?)", entity.getMaKH(), entity.getTenKH(), entity.getSdt(), entity.getEmail(), entity.getDiaChi());
    ...
    public void update(KhachHang entity) {
        jdbcHelper.update("update KHACHHANG set HOTEN=?,SODT=?,DIACHI=?,EMAIL=? where MAKHACHHANG = ?", entity.getTenKH(), entity.getSdt(), entity.getEmail(), entity.getDiaChi(), entity.getMaKH());
    }
     ...
     static boolean checkTrungMa(String key) {
        ...
        ArrayList<KhachHang> khachHangs = new ArrayList<>();
            try {
                ResultSet rs = jdbcHelper.query(query);
                while (rs.next()) {                    
                    KhachHang kh = new KhachHang(rs.getString("MAKHACHHANG"), rs.getString("HOTEN"), rs.getString("SODT"), rs.getString("DIACHI"), rs.getString("EMAIL"));
                    khachHangs.add(kh);
                }
                rs.getStatement().getConnection().close(); 
            } catch (Exception ex) {
