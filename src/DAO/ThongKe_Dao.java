...
public class ThongKe_Dao {
    ...
    public ArrayList<ThongKe> tongHoaDon() {
        ...
        try {
            ResultSet rs = jdbcHelper.query("select COUNT(MAHOADON) from HOADON where TRANGTHAI = 1");
            while (rs.next()) {
    ...
    public ArrayList<ThongKe> tongKhachHang() {
        ArrayList<ThongKe> listTk = new ArrayList<>();
        try {
            ResultSet rs = jdbcHelper.query("select COUNT(MAKHACHHANG) from KHACHHANG");
            while (rs.next()) {
                ThongKe tk = new ThongKe(rs.getInt(1));
                listTk.add(tk);
            }
...
//theo thang nam
    ...
    public ArrayList<ThongKe> tongHoaDonThangNam(Date batDau, Date ketThuc) {
        ...
        try {
            ResultSet rs = jdbcHelper.query("select COUNT(MAHOADON) from HOADON where TRANGTHAI = 1 and NGAYTAO between ? and ?", batDau, ketThuc);
            while (rs.next()) {
                ThongKe tk = new ThongKe(rs.getInt(1));
                listTk.add(tk);
            }
