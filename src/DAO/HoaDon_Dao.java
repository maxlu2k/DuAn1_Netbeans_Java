  ...
  ArrayList arrayList = new ArrayList();
    ...
    public ArrayList<HoaDon> select(int trangThai, int hinhthucthanhtoan, int hinhThucBanHang, Date BatDau, Date KetThuc) {
        ...
        try {
            ...
            ResultSet rs = jdbcHelper.query("SELECT * FROM HOADON WHERE TRANGTHAI != ? AND HINHTHUCTHANHTOAN != ? AND DANGGIAOHANG != ? AND NGAYTHANHTOAN BETWEEN ? AND ?", trangThai, hinhthucthanhtoan, hinhThucBanHang, BatDau, KetThuc);
            while (rs.next()) {
                String maKH = rs.getString("MAKHACHHANG");
                String tenKH = "";
                KhachHang kh = khachHang_Dao.selectByID(maKH);
     ...
     public ArrayList<HoaDon> selectNoDate(int trangThai, int hinhthucthanhtoan, int hinhThucBanHang) {
        ...
        try {
            ...
            ResultSet rs = jdbcHelper.query("SELECT * FROM HOADON WHERE TRANGTHAI != ? AND HINHTHUCTHANHTOAN != ? AND DANGGIAOHANG != ?", trangThai, hinhthucthanhtoan, hinhThucBanHang);
            while (rs.next()) {
                String maKH = rs.getString("MAKHACHHANG");
                String tenKH = "";
                KhachHang kh = khachHang_Dao.selectByID(maKH);
